...
// Abhängigkeit muss(wenn noch nicht vorhanden) zunächst mit "npm install @types/node"
  ...
  const server: http.Server = http.createServer(
    (request: http.IncomingMessage, response: http.ServerResponse) => {
      ...
      response.statusCode = 150; //hier wird der Status definiert: 200 heißt alles ist in Ordnung

      // Nun können wir noch Header für die Rückgabe Definieren
      response.setHeader("Content-Type", "date/plain"); //Hier Definieren wir den Rückgabetyp
      ...
      switch (url.pathname) {
        case "/": //Root-Pfad
          ...
          break;
        case "/convertDate": //Datum rückgeben
          let date: string = url.searchParams.get("eingabedatum"); //Auslesen eines Get-Parameters "eingabedatum"
        
          console.log(date); // Ausgabe der Angekommenen get-Parameters in der Server-Konsole
          response.write("Schön dich am " + date + ", abzudichten"); //Definieren der Rückgabe mit der Datums-Variable
          break;
        default:
